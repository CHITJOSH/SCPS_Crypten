...
class PaillierTensor(CrypTensor):
    ...
    def __add__(self, other: 'PaillierTensor') -> 'PaillierTensor':
        if self.public_key != other.public_key:
            raise ValueError("Public keys of operands must be the same.")
        return PaillierTensor(self.public_key)
...
if __name__ == "__main__":
    # Example usage:
    public_key, private_key = PaillierTensor.generate_key_pair()

    value1 = random.randint(0, 100)
    value2 = random.randint(0, 100)

    tensor1 = PaillierTensor(public_key)
    tensor2 = PaillierTensor(public_key)

    encrypted_sum = tensor1.encrypt(value1) + tensor2.encrypt(value2)
    print(encrypted_sum)
    decrypted_sum = PaillierTensor.decrypt(encrypted_sum, private_key)
